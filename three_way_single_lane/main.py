...
import three_way_single_lane.config as config
from three_way_single_lane.config import (
    sumo_cfg,
    output_path,
    step_length,
    simulation_steps,
    ...
    negotiation_end,
    intersection_start,
    intersection_end_east,
    intersection_end_west,
    intersection_end_south,
    ...
    exit_west,
    exit_south,
    straight_east_length,
    straight_west_first_length,
    straight_west_second_length,
    right_length,
    left_first_length,
    left_second_length,
    left_east_third_length,
    left_south_third_length,
    last_initial_east,
    last_initial_west,
    last_initial_south,
    last_position_east,
    ...
    for vehicle_id in vehicle_ids:
        ...
        if not(has_crossed.get(vehicle_id,False)):
            ...
            if route[0]==east_in:
                ...
                straight_area_entry = None
                straight_area_exit = None
                ...
                intersection_end_first = intersection_end_west
                intersection_end_second = intersection_end_south
                ...
                left_area1_entry = None
                left_area1_exit = None
                left_area2_entry = None
                left_area2_exit = None
                update_pos = -position[0]
                out_pos_first = position[0]
                out_pos_second = -position[1]
                intersection_end_first = intersection_end_east
                intersection_end_second = intersection_end_south
                ...
                intersection_end_first = intersection_end_east
                intersection_end_second = intersection_end_west
                    ...
                    # otherwise, we schedule it
                        ...
                        if route[1] == straight:
                            ...
                            while redo:
                                ...
                                while not algorithm_triggered[vehicle_id]:
                                    ...
                                    entry_time = do_check(
                                        ...
                                        mode="S1_east" if straight == west_out else "S1_west")
                                    if straight_area_entry != None:
                                        # Check for collisions on the second conflict zone (if present)
                                        entry_time = do_check(
                                            straight_area_entry,
                                            straight_area_exit,
                                            entry_time,
                                    ...
                                    if len(list_exit_straight) > 0:
                                        entry_time = after_check(
                                            ...
                                            decel,
                                            mode="S1_east" if route[0] == east_in else "S2"
                                        )
                            ...
                            if(straight_area_entry==None):
                                ...
                                bisect.insort(first_area_exit, entry_time + (straight_east_length + vehicle_length) / crossing_speed[vehicle_id])
                                bisect.insort(list_exit_straight,entry_time + (straight_east_length + vehicle_length) / crossing_speed[vehicle_id])
                                cross_speed_dict_straight[entry_time + (straight_east_length + vehicle_length) / crossing_speed[vehicle_id]] = crossing_speed[vehicle_id]
                            else:
                                bisect.insort(first_area_entry, entry_time)
                                bisect.insort(first_area_exit, entry_time + (straight_west_first_length + vehicle_length) / crossing_speed[vehicle_id])
                                bisect.insort(straight_area_entry,entry_time + straight_west_first_length / crossing_speed[vehicle_id])
                                bisect.insort(straight_area_exit,entry_time + (straight_west_second_length + vehicle_length) / crossing_speed[vehicle_id])
                                bisect.insort(list_exit_straight,entry_time + (straight_west_second_length + vehicle_length) / crossing_speed[vehicle_id])
                                cross_speed_dict_straight[entry_time + (straight_west_second_length + vehicle_length) / crossing_speed[vehicle_id]] = crossing_speed[vehicle_id]
                        ...
                        # The same procedure is performed here for vehicles turning at the intersection
                            ...
                            while redo:
                                ...
                                while not algorithm_triggered[vehicle_id]:
                                    ...
                                    # Check for collisions on conflict zones when turning left
                                        ...
                                        # Check for collisions on the third conflict zone when turning left (if present)
                                        entry_time = do_check(
                                            left_area2_entry,
                                            left_area2_exit,
                                            entry_time,
                                            crossing_speed[vehicle_id],
                                            mode="L3_south" if left==west_out else "L3_east"
                                        )
                                    if temp_entry != entry_time:
                                        algorithm_triggered[vehicle_id] = False
                                # If a new entry time is required by any of the checks, the EGO has to compute a new profile and the controller checks for collisions again
                            ...
                            # Store the conflict zone entry / exit times and other parameters when turning left
                                ...
                                if left == west_out:
                                    bisect.insort(left_area2_exit,entry_time + (left_south_third_length+vehicle_length)/crossing_speed[vehicle_id])
                                    bisect.insort(list_exit_left,entry_time + (left_south_third_length+vehicle_length)/crossing_speed[vehicle_id])
                                    cross_speed_dict_left[entry_time + (left_south_third_length+vehicle_length)/crossing_speed[vehicle_id]] = crossing_speed[vehicle_id]
                                else:
                                    bisect.insort(left_area2_exit,entry_time + (left_east_third_length+vehicle_length)/crossing_speed[vehicle_id])
                                    bisect.insort(list_exit_left,entry_time + (left_east_third_length+vehicle_length)/crossing_speed[vehicle_id])
                                    cross_speed_dict_left[entry_time + (left_east_third_length+vehicle_length)/crossing_speed[vehicle_id]] = crossing_speed[vehicle_id]
                                logger.info("Entry time of "+str(vehicle_id)+" at the end of the procedure: "+str(entry_time))
                                logger.info("----------------------------------------")
            ...
            if out_pos_first>intersection_end_first+vehicle_length or abs(out_pos_second)>intersection_end_second+vehicle_length:
